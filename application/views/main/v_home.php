		...
		<div class="col-sm-8 album text-muted">
			<div class="container">
				...
				<div class="row">	
					...
					$i = 0;
					$hasilharga = 0;
					$diskon_dropshipper = 0;

					foreach ($data_product as $items) {
						$i++;
						if($i < 6){

							if(!empty($data_user["user_lvl"]) && $data_user["user_lvl"] == 4 && $items->disc_dropshipper != 0){
								$diskon_dropshipper = $items->harga * $items->disc_dropshipper;
								$diskon_dropshipper = $diskon_dropshipper / 100;
								$hasilharga = $items->harga - $diskon_dropshipper;
							}else{
								$hasilharga = $items->harga;
							}
							...
							<div class="card-body">
							<center><p style="font-weight: 500; font-size: 25px;">Rp. '.number_format($hasilharga, 0, ',', '.').'</p></center>
							<p class="card-text">'.$items->nama_product.'anjingini</p>
							</div>
							<div class="card-footer bg-white">
							<a href='. base_url("product") .' class="btn btn-primary w-100">Lihat Produk</a>
							<a href='. base_url("shopping/addtocart/".$items->id_product) .' class="btn btn-primary w-100" style="margin-top: 5px;">Add to Cart</a>
							</div>
