...
class M_stok_notification extends CI_Model{
	...
	function select($product,$user){
		$this->db->select("*");
		$this->db->from("stok_notification");
	...
	function insert($data) {
		...
		if($this->db->insert("stok_notification", $data)){
			return true;
		}
	...
	function update($idproduct, $iduser, $data){
		...
		if($this->db->update('stok_notification')){
			return true;
		}
		return false;
	}
	...
	function delete($id_product, $id_user) {
		if($this->db->delete('stok_notification', array('id_product' => $id_product, 'id_user' => $id_user))){
			return true;
		}
		return false;
	}
